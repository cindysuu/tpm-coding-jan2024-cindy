...
class ChatLevelFeaturesCalculator:
    ...
    def apply_word_ttr(self) -> None:
        ...
        """
        self.chat_data["word_TTR"] = self.chat_data["message"].apply(get_word_TTR) # GETS ENTIRE COLUMN
    ...
    def apply_politeness(self) -> None:
        ...
        politeness_features = self.chat_data["message"].apply(get_politeness_strategies) # Dictionary with 21 items
        df = pd.DataFrame()
        for i, dictionary in enumerate(politeness_features):
            temp_df = pd.DataFrame([dictionary])
            df = pd.concat([df, temp_df], ignore_index=True)

        df.columns = df.columns.str.replace('feature_politeness_', '')
        self.chat_data = pd.concat([self.chat_data, df], axis=1)
